...
async function handleOrderUpdated(order, requestId) {
  ...
  // CRITICAL: Strict filtering - Bitrix API may return partial matches
  // Filter to only deals where UF_SHOPIFY_ORDER_ID exactly matches
  const deals = allDeals.filter(d => String(d.UF_SHOPIFY_ORDER_ID) === shopifyOrderId);
  const dealsCount = deals.length;

  console.log(`[SHOPIFY WEBHOOK] [${requestId}] Found ${dealsCount} exact match deal(s) for order ${shopifyOrderId} (filtered from ${allDeals.length} total)`);
  
  if (allDeals.length > dealsCount) {
    console.warn(`[SHOPIFY WEBHOOK] [${requestId}] ⚠️ Bitrix returned ${allDeals.length} deals, but only ${dealsCount} have exact UF_SHOPIFY_ORDER_ID match. Using exact matches only.`);
  }
  ...
  if (dealsCount === 0) {
    ...
    console.error(`[SHOPIFY WEBHOOK] [${requestId}] ⚠️ DATA ERROR: Found ${dealsCount} deals for order ${shopifyOrderId}!`);
